...
onReady(() => {
  ...
  pageInstance.animate(
    ...
    //动画效果，导航栏标题
    pageInstance.animate('.navbar .title', [{ color: 'transparent' }, { color: '#f8f8' }], 1000, {
      scrollSource: '#scroller',
      TimeRanges: 1000,
      startScrollOffset: 0,
      endScrollOffset: 50,
...
const onOrderPay = async () => {
  ...
  //关闭当前页面，在跳转支付结果页
  uni.redirectTo({ url: `/src/pagesOrder/payment?id=${query.id}` })
}
//是否为开发环境
    ...
    <template v-if="order">
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view v-if="order?.orderState === OrderState.DaiShouHuo" @tap="onOrderConfirm" class="button primary">
            确认收货
          </view>
          <!-- 待评价状态: 展示去评价 -->
