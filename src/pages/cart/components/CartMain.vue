...
const selectedCartListCount = computed(() => {
  return selectedCartListCount.value.reduce((sum: any, item: { count: any }) => sum + item.count, 0)
})
//计算选中总金额
const selectedCartListMoney = computed(() => {
  return selectedCartList.value
    .reduce(
      (sum: number, item: { count: number; nowPrice: number }) => sum + item.count * item.nowPrice,
      0,
    )
    .toFixed(2)
})
...
<template>
  <scroll-view scroll-y class="scroll-view">
    ...
    <!-- 猜你喜欢 -->
    <XtxGress ref="guessRef"></XtxGress>
    <!-- 底部占位空盒子 -->
